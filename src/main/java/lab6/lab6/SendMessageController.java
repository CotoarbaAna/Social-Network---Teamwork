...
public class SendMessageController implements Initializable {
    ...
    /**
     * Function for getting the username to send a message to
     */
    ...
    /**
     * Switches between SendMessage and Meniu pages
     * @throws Exception
     */
     ...
     * @throws Exception
     */
    public void onSeeClicked() throws Exception {
        setService();
        Integer ok = 0;
        if(sendToField.getText().isEmpty())
        {
            ...
            for (Username user : this.service.getFirendsFor(service.findUsername(username)))
                if (user.getUsername().equals(sendToField.getText())) {
                    ...
                    messagesController.passUser1(username);
                    messagesController.passUser2(user.getUsername());
                    messagesController.fillTable();
                    Stage stage = (Stage) SeeMessages.getScene().getWindow();
